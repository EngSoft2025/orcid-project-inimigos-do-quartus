...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [profileResponse, employmentsResponse, educationsResponse, worksResponse] = await Promise.allSettled([
      Promise.race([
        makeAuthenticatedOrcidRequest(`https://pub.orcid.org/v3.0/${orcidId}/person`),
        new Promise<Response>((_, reject) => setTimeout(() => reject(new Error('Profile timeout')), timeoutMs))
      ]),
      Promise.race([
        makeAuthenticatedOrcidRequest(`https://pub.orcid.org/v3.0/${orcidId}/employments`),
        new Promise<Response>((_, reject) => setTimeout(() => reject(new Error('Employments timeout')), timeoutMs))
      ]),
      Promise.race([
        makeAuthenticatedOrcidRequest(`https://pub.orcid.org/v3.0/${orcidId}/educations`),
        new Promise<Response>((_, reject) => setTimeout(() => reject(new Error('Educations timeout')), timeoutMs))
      ]),
      Promise.race([
        makeAuthenticatedOrcidRequest(`https://pub.orcid.org/v3.0/${orcidId}/works`),
        new Promise<Response>((_, reject) => setTimeout(() => reject(new Error('Works timeout')), timeoutMs))
      ])
    ])
    ...
    if (employmentsResponse.status === 'fulfilled' && employmentsResponse.value.ok) {
      try {
    ...
    if (worksResponse.status === 'fulfilled' && worksResponse.value.ok) {
      try {
        ...
        console.log(`Processing ${Math.min(workGroups.length, 50)} works for ${orcidId}`)
        
        // Process works in smaller batches for better performance
        const batchSize = 10
        const maxWorks = 50 // Limit to prevent timeout
        
        for (let i = 0; i < Math.min(workGroups.length, maxWorks); i += batchSize) {
            ...
            try {
              ...
              const workDetailResponse = await Promise.race([
                ...
                new Promise<Response>((_, reject) => 
                  setTimeout(() => reject(new Error('Work detail timeout')), 8000)
                )
              ])
              ...
              console.log(`Failed to get detail for work, using summary:`, errorMessage)
              return processWorkSummary(workSummary)
            }
          })
    ...
    console.log(`Extracted ${publications.length} publications from ORCID for ${orcidId}`)
    ...
    try {
      console.log(`Starting enrichment for ${name} with ${publications.length} publications`)
      ...
      if (enrichedData.publications && enrichedData.publications.length > 0) {
        enrichedData.publications = enrichedData.publications.map((pub: Publication) => ({
          ...pub,
          citations: formatDataOrDash(pub.citations)
        }))
        console.log(`Publications enriched: ${enrichedData.publications.filter((p: Publication) => p.citations !== "-" && p.citations !== 0).length} have citation data`)
      }
      
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error'
      console.log(`Enrichment failed for ${name}, using ORCID data only:`, errorMessage)
    ...
    console.log(`Profile enriched with data from: ${enrichedData.enhancedWith?.join(', ') || 'ORCID only'}`)
    ...
    // Cache the result
    detailedProfileCache.set(cacheKey, { data: profile, timestamp: Date.now() })
...
function processWorkSummary(workSummary: any): Publication {
  ...
  const year = workSummary["publication-date"]?.year?.value || new Date().getFullYear()
  const journal = workSummary["journal-title"]?.value || "Revista não informada"
